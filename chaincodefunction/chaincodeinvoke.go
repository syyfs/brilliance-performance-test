...
func Login(certPath string)  (tocken string , err error) {
...
func ChaincodeInvoke(i int , tocken string , cert string , invokeChan chan int, wg *sync.WaitGroup){
	fmt.Printf(" %c[%d;%d;%dm %s %c[0m \n", 0x1B, 1, 40, 31, tocken, 0x1B) // 红色
	...
	err = execute(cfgBytes , tocken, cert)
	if err != nil {
...
func execute(cfgBytes []byte,tocken string , cert string) error {
	if utils.GetHttpsEnable(){
		...
		err = clientDo(tocken , client , "POST" , url, cfgBytes  )
		if err != nil {
...
func clientDo(tocken string,client *http.Client ,method string, url string, cfgBytes []byte ) error {
	...
	fmt.Printf("\n %c[1;40;32m url:%s;res:%s;  token:%s\n %c[0m\n\n", 0x1B, url,string(body), tocken,0x1B)
	defer resp.Body.Close()
	return nil
}


func ChaincodeQuery(i int , tocken string, cert string,invokeChan chan int,wg *sync.WaitGroup)  {
	currentTime := time.Now().Unix()
	cci :=  &InvokeChaincodeCfg{
		ChannelId: "mychannel1",
		CcName: "mycc4",
...
func QueryChaincode(i int , tocken string, cert string,chanquery chan int,wg *sync.WaitGroup){
	currentTime := time.Now().Unix()
	cci :=  &InvokeChaincodeCfg{
		ChannelId: "mychannel1",
		CcName: "mycc4",
...
func query (cfgBytes []byte,tocken string , cert string){
	if utils.GetHttpsEnable(){
		client , err := HttpsReqClient(cert)
		if err != nil {
		...
		clientDo(tocken , client , "POST" , url, cfgBytes  )
